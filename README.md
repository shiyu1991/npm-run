...
- **服务管理**：服务节点显示 IP + 端口，可单独结束某一个服务（如 `concurrently` 同时起的 3 个服务只杀 1 个），也可单独重启某一个服务，其余不受影响；依赖主进程的 worker（如 Next.js dev）单独重启必败，其 ⟳ 自动显示为禁用图标并提示改用整脚本重启
...
- **Per-service control**: each service node shows IP + port with ⟳ restart and ✕ kill buttons; restart or kill just one service (e.g. one of three started via `concurrently`) without touching the others; workers that depend on the script main process (e.g. Next.js dev) get a disabled icon instead of ⟳, hinting at the whole-script restart
